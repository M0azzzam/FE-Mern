...
const ManageDevices = props => {
  ...
  const columnDefs = [
    ...
    { headerName: 'Device', field: 'name' },
    { headerName: 'Manufacturer', field: 'manufacturer.name' },
    {
      headerName: 'Colors', resizable: true, cellRendererFramework: (params) => (
        <>
          {getCommaSeparatedData(params)}
        </>
      )
    },
    {
      headerName: 'Show On POS', field: 'triggers.onPOS', cellRendererFramework: (params) => {
        return params.data && params.data.triggers && params.data.triggers.onPOS ? <Icon name='check' color='green' /> : <Icon name='cancel' color='red' />;
      }
    },
    {
      headerName: 'Show On Widget', field: 'triggers.onWidget', cellRendererFramework: (params) => {
        return params.data && params.data.triggers && params.data.triggers.onWidget ? <Icon name='check' color='green' /> : <Icon name='cancel' color='red' />;
      }
    },
    {
        ...
        <div style={{ float: 'right' }}>
          ...
          >
            <Dropdown.Menu>
              ...
              <Dropdown.Divider />
              <Dropdown.Item disabled={disable} onClick={() => handleDeleteSelectedItems(gridApiRef)} text='Delete Selected Devices' />
